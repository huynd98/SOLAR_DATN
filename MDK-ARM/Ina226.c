...
float INA226_getShuntV(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getShuntVReg(I2CHandler, DevAddress)*2.5/1000000);
}
float INA226_getBusV(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getBusVReg(I2CHandler, DevAddress)*1.25/1000);
}

float INA226_getCurrent_Discharge(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getCurrentReg(I2CHandler, DevAddress)*INA226_CURRENTLSB_DISCHARGE);
}

float INA226_getPower_Discharge(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getPowerReg(I2CHandler, DevAddress)*INA226_POWERLSB_DISCHARGE);
}

float INA226_getCurrent_Charge(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getCurrentReg(I2CHandler, DevAddress)*INA226_CURRENTLSB_CHARGE);
}

float INA226_getPower_Charge(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	return (INA226_getPowerReg(I2CHandler, DevAddress)*INA226_POWERLSB_CHARGE);
}
...
int16_t INA226_getShuntVReg(I2C_HandleTypeDef *I2CHandler, uint16_t DevAddress) {
	...
	HAL_I2C_Master_Transmit(I2CHandler,DevAddress, SentTable, 1, INA226_I2CTIMEOUT);
	if (HAL_I2C_Master_Receive(I2CHandler,DevAddress, ReceivedTable, 2, INA226_I2CTIMEOUT) != HAL_OK) return 0xFF;
	else return ((int16_t)ReceivedTable[0]<<8 | ReceivedTable[1]);
}
